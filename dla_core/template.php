...
global $theme_key;
if (db_is_active()) {
  dla_core_initialize_theme_settings($theme_key);
}
 ...
 */
function dla_core_preprocess_maintenance_page(&$vars) {
  if (db_is_active()) {
    dla_core_preprocess_page($vars);
  }
}
 ...
 * Page preprocessing
 */
function dla_core_preprocess_page(&$vars) {
  ...
  $left_width = ($vars['left']) ? theme_get_setting('left_width') : 0;
  $right_width = ($vars['right']) ? theme_get_setting('right_width') : 0;
  $vars['left_width'] = $grid_name . $left_width;
  $vars['main_group_width'] = $grid_name . ($grid_width - $left_width);
  // For nested elements in a fluid grid calculate % widths & add inline
  if ($grid_type == 'fluid') {
    $right_width = round(($right_width/($grid_width - $left_width)) * 100, 2);
    $vars['content_group_width'] = '" style="width:' . (100 - $right_width) . '%';
    $vars['right_width'] = '" style="width:' . $right_width . '%';
  }
  else {
    $vars['content_group_width'] = $grid_name . ($grid_width - ($left_width + $right_width));
    $vars['right_width'] = $grid_name . $right_width;
  }
  ...
  $themes = dla_core_theme_paths($theme_key);
  ...
  if (theme_get_setting('fix_css_limit') && !variable_get('preprocess_css', FALSE)) {
    ...
    if (dla_core_css_count($css) > (30 - $style_count)) {
      $styles = '';
      $suffix = "\n".'</style>'."\n";
 ...
 * Node preprocessing
 */
function dla_core_preprocess_node(&$vars) {
  ...
  if (module_exists('uc_product') && uc_product_is_product($vars) && $vars['template_files'][0] == 'node-product') {
    ...
    $vars['dla_core_uc_image'] = drupal_render($node->content['image']);
    $vars['dla_core_uc_body'] = drupal_render($node->content['body']);
    $vars['dla_core_uc_display_price'] = drupal_render($node->content['display_price']);
    $vars['dla_core_uc_add_to_cart'] = drupal_render($node->content['add_to_cart']);
    $vars['dla_core_uc_sell_price'] = drupal_render($node->content['sell_price']);
    $vars['dla_core_uc_cost'] = drupal_render($node->content['cost']);
    $vars['dla_core_uc_weight'] = (!empty($node->weight)) ? drupal_render($node->content['weight']) : '';   // Hide weight if empty
    if ($vars['dla_core_uc_weight'] == '') {
      unset($node->content['weight']);
    }
    $dimensions = !empty($node->height) && !empty($node->width) && !empty($node->length);                 // Hide dimensions if empty
    $vars['dla_core_uc_dimensions'] = ($dimensions) ? drupal_render($node->content['dimensions']) : '';
    if ($vars['dla_core_uc_dimensions'] == '') {
      unset($node->content['dimensions']);
    }
    $list_price = !empty($node->list_price) && $node->list_price > 0;                                     // Hide list price if empty or zero
    $vars['dla_core_uc_list_price'] = ($list_price) ? drupal_render($node->content['list_price']) : '';
    if ($vars['dla_core_uc_list_price'] == '') {
      unset($node->content['list_price']);
    }
    $vars['dla_core_uc_additional'] = drupal_render($node->content);                                        // Render remaining fields
  }
}
 ...
 */
function dla_core_preprocess_comment(&$vars) {
  global $user;
 ...
 */
function dla_core_preprocess_comment_wrapper(&$vars) {
 ...
 * Block preprocessing
 */
function dla_core_preprocess_block(&$vars) {
  global $theme_info, $user;
  static $regions, $left_width, $right_width, $grid_name, $grid_width, $grid_fixed;
  ...
  if (!isset($regions)) {
    ...
    $left_width = (block_list('left')) ? theme_get_setting('left_width') : 0;
    $right_width = (block_list('right')) ? theme_get_setting('right_width') : 0;
    $regions = dla_core_set_regions($grid_width, $left_width, $right_width);
  }
  ...
  if (!isset($vars['skinr']) || (strpos($vars['skinr'], $grid_name) === false)) {
    ...
    if ($vars['block']->region == 'left') {
      $width = ($grid_fixed) ? $left_width : $grid_width;  // Sidebar width or 100% (if fluid)
    }
    elseif ($vars['block']->region == 'right') {
      $width = ($grid_fixed) ? $right_width : $grid_width;  // Sidebar width or 100% (if fluid)
    }
    else {
  ...
  if (user_access('administer blocks', $user) && theme_get_setting('block_config_link')) {
    $vars['edit_links'] = '<div class="dla-core-edit">'. implode(' ', dla_core_edit_links($vars['block'])) .'</div>';
  }
}
 ...
 */
function dla_core_preprocess_views_view(&$vars) {
 ...
 */
function dla_core_preprocess_search_result(&$vars) {
 ...
 */
function dla_core_username($object) {
 ...
 */
function dla_core_file($element) {
 ...
 */
function dla_core_grid_row($element, $name, $class='', $width='', $extra='') {
  $output = '';
  $extra = ($extra) ? ' ' . $extra : '';
  if ($element) {
    if ($class == 'full-width') {
...
function dla_core_grid_block($element, $name) {
  $output = '';
  if ($element) {
 ...
 */
function dla_core_set_regions($grid_width, $left_width, $right_width) {
  $sidebar_total = $left_width + $right_width;
  $regions = array(
    ...
    'preface_bottom' => array('width' => $grid_width - $left_width, 'total' => count(block_list('preface_bottom')), 'count' => 0),
    'left' => array('width' => $left_width, 'total' => count(block_list('left')), 'count' => 0),
    ...
    'right' => array('width' => $right_width, 'total' => count(block_list('right')), 'count' => 0),
    'postscript_top' => array('width' => $grid_width - $left_width, 'total' => count(block_list('postscript_top')), 'count' => 0),
 ...
 */
function dla_core_edit_links($block) {
  ...
  if ($block->module == 'block') {
    ...
    $attributes = array('title' => t('edit the content of this Custom block (in @region)', $block_info), 'class' => 'dla-core-block-edit');
  }
  else {
    $text = t('configure block');
    $block_info = array('@type' => ucwords($block->module), '@region' => str_replace('_', ' ', $block->region));
    $attributes = array('title' => t('configure this @type block (in @region)', $block_info), 'class' => 'dla-core-block-config');
  }
  ...
  if (user_access('administer menu') && ($block->module == 'menu' || ($block->module == 'user' && $block->delta == 1))) {
    ...
    $attributes = array('title' => t('edit the menu of this @type block (in @region)', $block_info), 'class' => 'dla-core-edit-menu');
    $edit_links[] = l($text, $path, array('attributes' => $attributes, 'query' => $return));
  }
  return $edit_links;
}
 ...
 */
function dla_core_css_count($array) {
  $count = 0;
  foreach ($array as $item) {
    $count = (is_array($item)) ? $count + dla_core_css_count($item) : $count + 1;
  }
  return $count;
}
 ...
 */
function dla_core_theme_paths($theme) {
  $all_parents = array();
  $themes = list_themes();
 ...
 */
function dla_core_themesettings_link($prefix, $suffix, $text, $path, $options) {
